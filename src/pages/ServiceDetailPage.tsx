...
export const ServiceDetailPage = ({ serviceId }: { serviceId: number }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-950 transition-colors duration-500">
      ...
      <section className="relative h-[50vh] md:h-[60vh] lg:h-[75vh] w-full overflow-hidden bg-slate-900">
        ...
        <div className="absolute top-4 md:top-8 left-0 right-0 z-30">
          <Section className="flex justify-between items-center">
            <button
              ...
              className="group flex items-center gap-1.5 md:gap-2 px-3 md:px-5 py-2 md:py-2.5 bg-white/10 backdrop-blur-md border border-white/20 text-white rounded-full hover:bg-orange-600 transition-all shadow-xl"
            >
              <ArrowLeft size={16} className="md:w-[18px] md:h-[18px] group-hover:-translate-x-1 transition-transform" />
              <span className="text-xs md:text-sm font-bold uppercase tracking-wider">Layanan</span>
            </button>
              ...
              {service.images.map((_, i) => (
                <button
                  ...
                  className={`h-1 md:h-1.5 transition-all duration-500 rounded-full ${
                    i === currentImageIndex ? 'w-6 md:w-8 bg-orange-500' : 'w-1.5 md:w-2 bg-white/40'
                  }`}
                />
              ))}
            </div>
        ...
        <div className="absolute inset-0 flex items-end pb-8 md:pb-12 lg:pb-20">
          <Section>
            <motion.div
            ...
            >
              <div className="flex items-center gap-2 md:gap-3 mb-3 md:mb-6">
                <span className="px-2 md:px-3 py-0.5 md:py-1 bg-orange-600 text-white text-[8px] md:text-[10px] font-black uppercase tracking-[0.15em] md:tracking-[0.2em] rounded">Featured Service</span>
                <div className="h-[1px] w-8 md:w-12 bg-orange-500/50" />
              </div>
              <h1 className="text-3xl md:text-5xl lg:text-7xl font-black text-white mb-3 md:mb-6 tracking-tighter uppercase leading-tight">
                {service.title.split(' ')[0]} <span className="text-orange-500">{service.title.split(' ').slice(1).join(' ')}</span>
              </h1>
              <p className="text-sm md:text-xl lg:text-2xl text-slate-200 font-light leading-relaxed max-w-2xl">
                {service.description}
              </p>
            </motion.div>
          </Section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-6">
            {[
              ...
              <motion.div
                ...
                className="bg-white dark:bg-slate-900 p-4 md:p-8 rounded-2xl md:rounded-[2rem] shadow-2xl border border-slate-100 dark:border-slate-800 flex items-center gap-3 md:gap-6"
              >
                <div className={`p-2.5 md:p-4 rounded-xl md:rounded-2xl ${stat.bg} ${stat.color}`}>
                  <stat.icon size={20} className="md:w-7 md:h-7" />
                </div>
                <div>
                  <p className="text-[9px] md:text-xs font-bold text-slate-400 uppercase tracking-widest mb-0.5 md:mb-1">{stat.label}</p>
                  <p className="text-base md:text-xl font-black text-slate-900 dark:text-white">{stat.value}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <Section className="py-12 md:py-24">
        <div className="grid lg:grid-cols-12 gap-8 md:gap-16 items-start">
          ...
          <div className="lg:col-span-7 space-y-8 md:space-y-12">
            <div className="space-y-4 md:space-y-6">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-xs md:text-sm font-black uppercase tracking-[0.15em] md:tracking-[0.2em] text-orange-600">Deep Insights</h2>
              </div>
              ...
              <div className="p-5 md:p-8 bg-slate-50 dark:bg-slate-900 rounded-2xl md:rounded-[2.5rem] border-l-4 border-orange-600 italic text-sm md:text-base text-slate-700 dark:text-slate-300">
            ...
            <div className="grid md:grid-cols-2 gap-6 md:gap-8">
              <div className="space-y-4 md:space-y-6">
                <h4 className="text-lg md:text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2 md:gap-3">
                  <PackageCheck className="text-orange-600" size={20} /> Spesifikasi
                </h4>
                <ul className="space-y-3 md:space-y-4">
                  {service.features.map((f, i) => (
                    <li key={i} className="flex items-center gap-2 md:gap-3 text-sm md:text-base text-slate-600 dark:text-slate-400">
                      <div className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-orange-600 flex-shrink-0" />
                      {f}
                    </li>
                  ))}
                </ul>
              ...
              <div className="space-y-4 md:space-y-6">
                <h4 className="text-lg md:text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2 md:gap-3">
                  <ShieldCheck className="text-green-600" size={20} /> Benefit
                </h4>
                <ul className="space-y-3 md:space-y-4">
          ...
          <div className="lg:col-span-5 lg:sticky lg:top-32">
            <div className="bg-slate-900 p-6 md:p-10 rounded-3xl md:rounded-[3rem] text-white relative overflow-hidden">
              <div className="absolute top-0 right-0 w-24 h-24 md:w-32 md:h-32 bg-orange-600/20 rounded-full blur-3xl" />
              <div className="relative z-10 space-y-5 md:space-y-8">
                <h4 className="text-2xl md:text-3xl font-black leading-tight">Konsultasi <br /> Gratis Sekarang</h4>
                <p className="text-sm md:text-base text-slate-400">Hubungi pakar logistik kami untuk mendapatkan penawaran harga khusus sesuai volume pengiriman Anda.</p>
                <div className="space-y-3 md:space-y-4">
                  <button 
                    onClick={() => setCurrentPage('contact')}
                    className="w-full py-4 md:py-5 bg-orange-600 hover:bg-orange-700 text-white font-black rounded-xl md:rounded-2xl transition-all flex items-center justify-center gap-2 md:gap-3 group shadow-xl shadow-orange-600/20 text-sm md:text-base"
                  >
                    Minta Penawaran <ArrowRight size={18} className="group-hover:translate-x-2 transition-transform" />
                  </button>
                  <p className="text-center text-[10px] md:text-xs text-slate-500 font-bold uppercase tracking-widest">Respon cepat dalam 15 menit</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="bg-slate-50 dark:bg-slate-900/50 py-6 md:py-12 border-y border-slate-100 dark:border-slate-800">
        <Section>
          <div className="flex flex-col md:flex-row justify-between items-center gap-6 md:gap-8">
            <button 
              onClick={() => { setServiceDetailId(prevService); window.scrollTo(0,0); }}
              className="flex items-center gap-3 md:gap-4 group w-full md:w-auto"
            >
              <div className="w-10 h-10 md:w-12 md:h-12 rounded-full border border-slate-200 dark:border-slate-700 flex items-center justify-center group-hover:bg-orange-600 group-hover:text-white transition-all flex-shrink-0">
                <ChevronLeft size={20} />
              </div>
              <div className="text-left">
                <p className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest">Previous Service</p>
                <p className="text-sm md:text-lg font-bold text-slate-900 dark:text-white">{services[prevService].title}</p>
              </div>
            </button>
            ...
            <button 
              ...
              className="flex items-center gap-3 md:gap-4 group text-right w-full md:w-auto justify-end"
            >
              <div className="text-right">
                <p className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest">Next Service</p>
                <p className="text-sm md:text-lg font-bold text-slate-900 dark:text-white">{services[nextService].title}</p>
              </div>
              <div className="w-10 h-10 md:w-12 md:h-12 rounded-full border border-slate-200 dark:border-slate-700 flex items-center justify-center group-hover:bg-orange-600 group-hover:text-white transition-all flex-shrink-0">
